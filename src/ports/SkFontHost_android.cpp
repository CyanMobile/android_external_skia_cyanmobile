...
public:
    FamilyTypeface(Style style, bool sysFont, SkTypeface* familyMember, const int useFallbackFontsEx)
    : SkTypefaceEx(style, sk_atomic_inc(&gUniqueFontID) + 1, sysFont) {
...
public:
    StreamTypeface(Style style, bool sysFont, SkTypeface* familyMember,
                   SkStream* stream)
    : INHERITED(style, sysFont, familyMember, DEFAULT_FALLBACK_NO) {
        SkASSERT(stream);
        stream->ref();
        fStream = stream;
    }
...
public:
    FileTypeface(Style style, bool sysFont, SkTypeface* familyMember,
                 const char path[], const int useFallbackFontsEx)
    : INHERITED(style, sysFont, familyMember, useFallbackFontsEx) {
        SkString fullpath;
